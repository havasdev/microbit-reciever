...
public class Utility {
    ...
    public static String getStringWithEncode(byte[] bytes) throws UnsupportedEncodingException {
        return new String(bytes, getCharset(bytes));
    }


    public static String getCharset(byte[] bytes) throws UnsupportedEncodingException {
        if(Arrays.equals(new String(bytes, "UTF-8").getBytes("UTF-8"), bytes)){
            return "UTF-8";
        }
        if(Arrays.equals(new String(bytes, "SJIS").getBytes("SJIS"), bytes)) {
            return "SJIS";
        }
        if(Arrays.equals(new String(bytes, "UTF-16").getBytes("UTF-16"), bytes)){
            return "UTF-16";
        }
        if(Arrays.equals(new String(bytes, "ASCII").getBytes("ASCII"), bytes)){
            return "ASCII";
        }
    ...
    public static boolean isJson(String txt){
        Object object = null;
        try {
            object = new JSONTokener(txt).nextValue();
