...
import com.inov.semina.microbitreciever.Settings;
import com.inov.semina.microbitreciever.Utility;
import com.inov.semina.microbitreciever.bluetooth.BleScanner;
import com.inov.semina.microbitreciever.bluetooth.BleScannerFactory;
import com.inov.semina.microbitreciever.bluetooth.ScanResultsConsumer;
...
public class MainActivity extends AppCompatActivity implements ScanResultsConsumer {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        showMsg(Utility.htmlColorGreen("スキャン準備"));
        ...
        listView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                if (device.getBondState() == BluetoothDevice.BOND_NONE && Settings.getInstance().isFilter_unpaired_devices()) {
                    device.createBond();
                    showMsg(Utility.htmlColorRed("選択したmicro:bitはすでにペアリング中です"));
                    return;
                }
                try {
                ...
                Intent intent = new Intent(MainActivity.this, UartAvmActivity.class);
                intent.putExtra(UartAvmActivity.EXTRA_NAME, device.getName());
                intent.putExtra(UartAvmActivity.EXTRA_ID, device.getAddress());
...
*/
    ...
    public void onScan(View view) {
        ...
        if (!ble_scanner.isScanning()) {
            ...
            showMsg(Utility.htmlColorGreen("スキャンを終了しています"));
            ble_scanner.stopScanning();
        }
    }
    ...
    private void startScanning() {
        if (permissions_granted) {
            ...
            showMsg(Utility.htmlColorRed("Bluetooth操作権限がありません。"));
        }
    }
    ...
    private void setScanState(boolean value) {
        ble_scanning = value;
        ((Button) this.findViewById(R.id.scanButton)).setText(value ? Constants.STOP_SCANNING : "Find BBC micro:bits");
    }
    ...
    public void scanningStopped() {
        setScanState(false);
        if (device_count > 0) {
            showMsg(Utility.htmlColorGreen("スキャン完了"));
        } else {
            ...
            if (BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action)) {
                BluetoothDevice device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
                        if (device.getBondState() == BluetoothDevice.BOND_NONE) {
                            showMsg(Utility.htmlColorRed("ペアリングできてません"));
                        } else if (device.getBondState() == BluetoothDevice.BOND_BONDING) {
                            showMsg(Utility.htmlColorGreen("ペアリングしています"));
                        } else {
                            showMsg(Utility.htmlColorGreen("ペアリングできました"));
                        }
                }
            }
        };
    ...
    private String getScanningMessage() {
        if (Settings.getInstance().isFilter_unpaired_devices()) {
            return "ペアリング中のmicro:bitsを探しています";
        } else {
            return "全てのmicro:bitsを探しています";
